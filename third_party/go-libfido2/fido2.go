...
func (d *Device) MakeCredential(
	...
	if len(opts.ClientDataJSON) > 0 {
		if cErr := C.fido_cred_set_clientdata(cCred, cBytes(opts.ClientDataJSON), cLen(opts.ClientDataJSON)); cErr != C.FIDO_OK {
			return nil, errors.Wrap(errFromCode(cErr), "failed to set client data")
		}
	} else if cErr := C.fido_cred_set_clientdata_hash(cCred, cBytes(clientDataHash), cLen(clientDataHash)); cErr != C.FIDO_OK {
...
func (d *Device) Assertion(
	...
	if len(opts.ClientDataJSON) > 0 {
		if cErr := C.fido_assert_set_clientdata(cAssert, cBytes(opts.ClientDataJSON), cLen(opts.ClientDataJSON)); cErr != C.FIDO_OK {
			return nil, errors.Wrapf(errFromCode(cErr), "failed to set client data")
		}
	} else if cErr := C.fido_assert_set_clientdata_hash(cAssert, cBytes(clientDataHash), cLen(clientDataHash)); cErr != C.FIDO_OK {
